...
fun Markdown(
    ...
    scope.launch {
        val parsedTree = MarkdownParser(CommonMarkFlavourDescriptor()).buildMarkdownTreeFromString(content)
...
private fun RenderComponent(
    ...
    Surface {
        SelectionContainer {
            when (item) {
                is Code -> Code(
                    text = item.codeBlock, config.colors?.get(
                        MarkdownConfig.CODE_BACKGROUND_COLOR
                    ) ?: Color.Gray, config.colors?.get(MarkdownConfig.CODE_BLOCK_TEXT_COLOR) ?: Color.White
                )

                is ItalicText -> ItalicText(
                    text = item.text, config.colors?.get(
                ...
                is Link -> Link(
                    item.text, item.link, config.colors?.get(
                        MarkdownConfig.LINKS_COLOR
                    ) ?: Color.Black, config.isLinksClickable, onLinkClickListener
                )
                ...
                is MarkdownStyledTextComponent -> MKStyledText(
                    item.text, item.layer, color = config.colors?.get(
