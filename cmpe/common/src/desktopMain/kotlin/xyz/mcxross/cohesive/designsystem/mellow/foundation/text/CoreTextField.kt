...
package xyz.mcxross.cohesive.designsystem.mellow.foundation.text
...
import xyz.mcxross.cohesive.designsystem.mellow.ContextMenuArea
import xyz.mcxross.cohesive.designsystem.mellow.foundation.text.selection.SimpleLayout
import xyz.mcxross.cohesive.designsystem.mellow.foundation.text.selection.TextFieldSelectionHandle
import xyz.mcxross.cohesive.designsystem.mellow.foundation.text.selection.TextFieldSelectionManager
import xyz.mcxross.cohesive.designsystem.mellow.foundation.text.selection.isSelectionHandleInVisibleBound
 ...
 * for this CoreTextField. You can create and pass in your own remembered [MutableInteractionSource]
 * if you want to observe [Interaction]s and customize the appearance / behavior of this
 * CoreTextField in different [Interaction]s.
 ...
 * @param decorationBox Composable lambda that allows to plus decorations around text field, such as
 * icon, placeholder, helper messages or similar, and automatically increase the hit target area of
 * the text field. To allow you to control the placement of the inner text field relative to your
 * decorations, the text field implementation will pass in a framework-controlled composable
 * parameter "innerTextField" to the decorationBox lambda you provide. You must call innerTextField
 * exactly once.
 */
@Composable
...
internal fun CoreTextField(
    ...
    Modifier.semantics(true) {
      ...
      setSelection { start, end, traversalMode ->
        if (!enabled) {
          ...
          start.coerceAtMost(end) >= 0 && start.coerceAtLeast(end) <= value.annotatedString.length
        ) {
  ...
  val decorationBoxModifier =
    modifier
      .textFieldScrollable(scrollerPosition, interactionSource, enabled, onScroll)
      .then(pointerModifier)
      .then(semanticsModifier)
      .then(focusModifier)
      .previewKeyEventToDeselectOnBack(state, manager)
      .then(textKeyInputModifier)
      .onGloballyPositioned { state.layoutResult?.decorationBoxCoordinates = it }
  ...
  CoreTextFieldRootBox(decorationBoxModifier, manager) {
    decorationBox {
      ...
      val coreTextFieldModifier =
        Modifier.maxLinesHeight(maxLines, textStyle)
          .textFieldScroll(
            scrollerPosition,
            value,
      ...
      SimpleLayout(coreTextFieldModifier) {
        ...
        if (state.handleState == HandleState.Cursor && !readOnly && showHandle) {
...
internal enum class HandleState {
   ...
   * user long click on the text and start selection, the TextField will exit this state and enters
   * [HandleState.Selection] state. If the user tap on the text, the TextField will exit this state
   * and enters [HandleState.Cursor] state.
   */
  None,

  /**
   * Selection handle is displayed for this TextField. User can drag the selection handle to change
   * the selected text. If the user start editing the text, the TextField will exit this state and
   * enters [HandleState.None] state. If the user tap on the text, the TextField will exit this
   * state and enters [HandleState.Cursor] state.
   */
  Selection,
...
@OptIn(InternalFoundationTextApi::class)
internal class TextFieldState(var textDelegate: TextDelegate) {
   ...
   * into text's coordinate system. For example, if you plus a gesture on top of the decoration box
   * and want to know the character in text for the given touch offset on decoration box. When you
   * don't need to shift the touch offset, you should be using `layoutResult.value` which omits the
   * proxy and calls the layout result directly. This is needed when you work with the text
   * directly, and not the decoration box. For example, cursor modifier gets position using the
   * [TextFieldValue.selection] value which corresponds to the text directly, and therefore does not
   * require the translation.
   */
  var layoutResult: TextLayoutResultProxy? = null

  /**
   * The gesture detector state, to indicate whether current state is selection, cursor or editing.
   *
   * In the none state, no selection or cursor handle is shown, only the cursor is shown. TextField
   * is initially in this state. To enter this state, input anything from the keyboard and modify
   * the text.
   *
   ...
   *
   * In the cursor state, no selection is shown, and the cursor and the cursor handle are shown. To
   * enter the cursor state, tap anywhere within the TextField.(The TextField will stay in the edit
   * state if the current text is empty.) In this mode, finger movement on the screen moves the
   * cursor.
   */
  var handleState by mutableStateOf(HandleState.None)

  /**
   * A flag to check if the selection start or end handle is being dragged. If this value is true,
   * then onPress will not select any text. This value will be set to true when either handle is
   * being dragged, and be reset to false when the dragging is stopped.
   */
  ...
  fun update(
    ...
    textDelegate =
      updateTextDelegate(
        ...
        softWrap = softWrap,
        density = density,
        resourceLoader = fontFamilyResolver,
...
internal fun TextFieldCursorHandle(manager: TextFieldSelectionManager) {
  ...
  manager.state?.layoutResult?.value?.let {
    ...
    val x = with(LocalDensity.current) { cursorRect.left + DefaultCursorThickness.toPx() / 2 }
    CursorHandle(
      handlePosition = Offset(x, cursorRect.bottom),
      modifier = Modifier.pointerInput(observer) { detectDragGesturesWithObserver(observer) },
      content = null,
    )
  }
}
