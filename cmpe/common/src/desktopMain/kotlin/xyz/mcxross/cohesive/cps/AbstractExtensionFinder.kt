...
abstract class AbstractExtensionFinder(val pluginManager: PluginManager) :
   ...
   * 'xyz.mcxross.cohesive.cps.CohesivePlugin') It's responsible for System Plugins
   ...
   * 'xyz.mcxross.cohesive.cps.CohesivePlugin')
   * @return a [Map] of extensions or an empty [Map] if the file is not found
   */
  ...
  override fun <T> find(type: Class<T>, pluginId: String): List<ExtensionWrapper<T>> {
    ...
    if (pluginId != "cohesive") {
      ...
      if (xyz.mcxross.cohesive.cps.PluginState.STARTED != pluginWrapper.pluginState) {
        return result
      }
      ...
      try {
        if (isCheckForExtensionDependencies()) {
          ...
          getExtensionInfo(className, classLoader).plugins.forEach {
            ...
            if (xyz.mcxross.cohesive.cps.PluginState.STARTED != requiredPlugin.pluginState) {
        ...
        if (type.isAssignableFrom(extensionClass)) {
          ...
          if (xyz.mcxross.cohesive.cps.RuntimeMode.DEVELOPMENT == pluginManager.runtimeMode) {
  ...
  override fun pluginStateChanged(event: PluginStateEvent) {
    ...
    if (
      checkForExtensionDependencies == null &&
        xyz.mcxross.cohesive.cps.PluginState.STARTED == event.pluginState
    ) {
